    ...
    <Popover className="relative flex justify-end">
      {({ open }) => (
        <>
          ...
          <div className="w-0">
            <div className="relative flex justify-center items-center text-black z-0 bg-gray-300 w-6 h-6 text-xs rounded-full -left-3.5 cart-counter">
              {solutions.length >= 99 ? 99 : solutions.length}
            </div>
          </div>
          <Transition
            as={Fragment}
                ...
                <div className="overflow-y-scroll z-50 scrollbar-thin scrollbar-track-rounded-full relative flex flex-col justify-start gap-2 bg-white pt-4 px-4 cart-upper">
                  {solutions.map((item) => (
                    <div key={item.name} className="flex flex-col">
                      ...
                      {solutions.length === 1 ||
                      item.name ===
                        solutions[solutions.length - 1].name ? null : (
