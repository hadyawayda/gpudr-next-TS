        ...
        <div id='headlessui-dialog-19' className='fixed inset-0 z-50 overflow-hidden'>
            ...
            <div className="fixed inset-0 flex items-start justify-end overflow-y-auto translate-x-0">
                <div className="min-h-full w-[min(20rem,calc(100vw-theme(spacing.10)))] bg-white shadow-2x1 ring-1 ring-black/10 transition">
                    ...
                    <button type="button" class="absolute top-5 right-6 flex h-8 w-8 items-center justify-center" tabindex="0"><span class="sr-only">Close navigation</span><svg class="h-3.5 w-3.5 overflow-visible stroke-slate-900" fill="none" stroke-width="1.5" stroke-linecap="round" xmlns="http://www.w3.org/2000/svg"><path d="M0 0L14 14M14 0L0 14"></path></svg></button>
                    <nav class="divide-y divide-slate-900/10 text-base leading-7 text-slate-900">
                        <div class="py-6 px-8">
                            <Link class="block w-9 overflow-hidden" href="https://gpudoctor.com">
                        ...
                        <div class="py-6 px-8">
                            <div class="-my-2 items-start space-y-2">
                                <Link class="block w-full py-2 font-semibold" href="/components">Components</Link>
                                <Link class="flex w-full items-center py-2 font-semibold" href="/templates">Templates<div class="ml-2 rounded-full bg-sky-500 px-1.5 py-0.5 text-xs text-white sm:hidden">New</div></Link>
                                <Link class="block w-full py-2 font-semibold" href="/documentation">Documentation</Link>
                            </div>
                        </div>
                        <div class="py-6 px-8">
                            <div class="-my-2 space-y-4">
                                <Link class="block w-full py-2 font-semibold" href="/login">Sign in</Link>
                                <Link class="inline-flex justify-center rounded-lg text-sm font-semibold py-3 px-4 bg-slate-900 text-white hover:bg-slate-700 w-full" href="/all-access"><span>Get all-access <span aria-hidden="true">→</span></span></Link>
                            </div>
                        </div>
                    </nav>
                </div>
